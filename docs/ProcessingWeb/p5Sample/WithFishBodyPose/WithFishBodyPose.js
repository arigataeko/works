...
function drawParticles() { //泡を描く
  for (let i = 0; i < poses.length; i++) {  // 検知されたポーズごとにループ
    ...
    if (poses[i].confidence > 0.2) {  //ポーズの精度の平均が0.2以上の時だけ
      let awaX = pose.keypoints[0].x;  //x
      let awaY = pose.keypoints[0].y + 30;  //y
...
function drowSnorkelEye() { //シュノーケル(ゴーグル部分)を描く
  for (let i = 0; i < poses.length; i++) {
    ...
    if (pose.confidence > 0.2) {
      　//ポーズの精度が0.2以上のときだけ
        let noseX = pose.keypoints[0].x;
      　//鼻のx座標
        let noseY = pose.keypoints[0].y;
      　//鼻のy座標
        let eyeLX = pose.keypoints[1].x;
      　//左目のx座標
        let eyeLY = pose.keypoints[1].y;
      　//左目のy座標
        let eyeRX = pose.keypoints[2].x;
      　//右目のx座標
        let eyeRY = pose.keypoints[2].y;
      　//右目のy座標
